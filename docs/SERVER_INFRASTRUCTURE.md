# Server infrastructure – gunduata.tech

The live website and API are served at **https://gunduata.tech**. This document records the server layout and how to access machines for deployment and maintenance.
...
| Server   | IP             | Role                                                                 |
|----------|----------------|----------------------------------------------------------------------|
| **LB**   | `72.62.226.41` | Nginx, **gunduata.tech**; forwards to App 1/2/3 on port **8001**     |
| **App 1**| `72.61.254.71` | web, game_timer, bet_worker                                          |
| **App 2**| `72.61.254.74` | web, game_timer, bet_worker, **Redis**                               |
| **App 3**| `72.62.226.41` | *(now used as LB — see above)*                                       |
| **DB**   | `72.61.255.231`| PostgreSQL, port **6432**                                            |

### Details

- **Domain:** `gunduata.tech` is configured on the Load Balancer (LB).
- **Traffic:** LB proxies to app servers **71** and **74** on **port 8001**.
...
- **Frontend / API base:** The website is configured to use `https://gunduata.tech/api/` as the API base (see [website/README.md](../website/README.md)).
