...
public class UnityPlayerGameActivity extends GameActivity implements IUnityPlayerLifecycleEvents, IUnityPermissionRequestSupport, IUnityPlayerSupport
{
    ...
    private static final String METHOD = "SetAccessAndRefreshTokens";
    ...
    /** Send access/refresh tokens from Intent to Unity GameManager to bypass login when launched from Sikwin. */
    private void sendTokensToUnityWhenReady() {
        if (mTokensSentToUnity) return;
        Intent intent = getIntent();
        String access = intent != null ? intent.getStringExtra("access") : null;
        String refresh = intent != null ? intent.getStringExtra("refresh") : null;
        if ((access == null || access.isEmpty()) && (refresh == null || refresh.isEmpty())) {
            try {
                ...
                Log.d(TAG, "UnityTokenHolder not available: " + t.getMessage());
            }
        }
        ...
        new Handler(Looper.getMainLooper()).postDelayed(() -> {
            try {
                String json = new JSONObject().put("access", acc).put("refresh", ref).toString();
                UnityPlayer.UnitySendMessage(GAMEOBJECT, METHOD, json);
                mTokensSentToUnity = true;
                Log.d(TAG, "Sent tokens to GameManager.SetAccessAndRefreshTokens");
            } catch (Throwable t) {
                Log.e(TAG, "Send tokens to Unity failed: " + t.getMessage());
            }
        }, 1500);
    }
