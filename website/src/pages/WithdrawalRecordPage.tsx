...
export function WithdrawalRecordPage() {
  ...
  const filtered =
    filter === 'Success'
      ...
      : filter === 'Failed'
        ? list.filter((w) => w.status === 'REJECTED')
        : list;
    ...
    <div className="mobile-frame min-h-dvh bg-[#1A1A1A]">
      ...
      <header className="sticky top-0 z-40 bg-[#1A1A1A]/95 backdrop-blur">
        <div className="mx-auto flex max-w-[430px] items-center gap-3 px-4 py-4">
          ...
          <h1 className="flex-1 text-center text-xl font-bold text-primaryYellow">Withdrawal Record</h1>
          <div className="w-11" />
        </div>
      </header>
        ...
        {loading ? (
          ...
          <div className="mt-4 space-y-3 pb-8">
            {filtered.map((w) => {
              ...
              return (
                <div key={w.id} className="rounded-xl bg-[#2a2a2a] p-4">
                  <div className="flex justify-between gap-4">
                    <div className="min-w-0 flex-1">
                      <p className="font-bold">
                        <span className={isRejected ? 'text-red-500' : 'text-white'}>Withdrawal #{w.id}</span>
                        <span className={w.status === 'COMPLETED' ? ' text-green-500' : isRejected ? ' text-red-500' : ' text-white'}>
                          {' '}({w.status})
                        </span>
                      </p>
                      <p className="mt-1 text-xs text-white/80">{w.created_at}</p>
                      {isRejected && w.admin_note && (
