...
public class DiceAndBox : MonoBehaviour
{
    ...
    /// <summary>Apply lightning effect to dice whose value appears more than 2 times (from frequency API).</summary>
    public void ApplyLightningForNumbers(HashSet<int> numbersWithHighFrequency)
    {
        if (diceRoller != null)
            diceRoller.ApplyLightningForNumbers(numbersWithHighFrequency);
    }
